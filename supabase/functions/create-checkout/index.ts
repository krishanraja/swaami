...
serve(async (req) => {
  ...
  try {
    const supabaseClient = createSupabaseClient({ useServiceRole: false });
    ...
    // Swaami+ price ID
    const priceId = "price_1SdNprHf9EbsrhltVKhL2PH6";
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `${req.headers.get("origin")}/app?upgrade=cancelled`,
      metadata: {
        user_id: user.id,
      },
    });
